...
- (Application: GUI) The Node-RED dashboard's "Administration" page now provides an embedded viewer for the operating system's logs, in a new "Operating System" section.

### Changed

- (User-facing change; Application: GUI) Previously, the Node-RED dashboard was accessed on the path `/ui` on port 1880, e.g. with URLs like `http://planktoscope.local:1880/ui` or `http://192.168.4.1:1880/ui`; now, it must be accessed on the path `/ps/node-red-v2/ui` on port 80, e.g. with URLs like `http://planktoscope.local/ps/node-red-v2/ui` or `http://192.168.4.1/ps/node-red-v2/ui`. You can also access the Node-RED dashboard at the same path on port 1880, but the embedded image streams and file gallery will not be properly displayed.
- (User-facing change; Application: GUI) Previously, the Node-RED dashboard's "Administration" page merged log entries from every component of the Python backend and the Node-RED frontend. Now, log entries are displayed separately for the Python backend's hardware controller, the Python backend's data processing segmenter, and the Node-RED dashboard.
