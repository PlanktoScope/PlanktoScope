...
- (System: administration, troubleshooting) [w3m](https://en.wikipedia.org/wiki/W3m) and [browsh](https://www.brow.sh/) are now installed, enabling terminal-based interaction with wifi network captive portals to obtain internet access on the PlanktoScope.
- (System: networking) [ufw](https://en.wikipedia.org/wiki/Uncomplicated_Firewall) is now installed, though it is disabled by default. Note: in the future, ufw may be removed in favor of firewall management using [firewalld](https://packages.debian.org/buster/firewalld).
