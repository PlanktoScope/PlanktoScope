...
- (System: administration) [Docker](https://www.docker.com/) is now installed. Upon the first boot-up of the SD card, Docker Swarm mode is initialized; for now, only a single-member swarm (consisting of the PlanktoScope's Raspberry Pi itself) is supported. However, the use of Docker Swarm mode should be considered experimental - we might decide in a later release to run Docker without Swarm mode.
