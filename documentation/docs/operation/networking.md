...
- mDNS (port 5353), whose access is provided by a Forklift package deployment named `host/networking/avahi`: restricting access to this will prevent devices in the `public` zone from discovering the PlanktoScope's IP address(es) by querying the PlanktoScope's mDNS hostnames (i.e. hostnames ending in `.local`, such as `planktoscope.local`).
