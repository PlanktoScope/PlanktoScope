...
async function updateDnsmasq(hostname) {
  let hosts = await readFile(
    fileURLToPath(import.meta.resolve("../os/network/dnsmasq-shared.properties")),
    "utf8",
  )
  hosts = hosts.replaceAll("raspberrypi", hostname)
  await writeFile("/etc/NetworkManager/dnsmasq-shared.d/40-config.conf", hosts)
}
