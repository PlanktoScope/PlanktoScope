...
import { opendir, readFile, access, constants, readdir } from "fs/promises"
...
export function getGalleryPath(path) {
  if (!path.startsWith("/home/pi/data/")) return null
  return join("/ps/data/browse/files/", relative("/home/pi/data/", path))
}
...
async function getSegmentationFromPath(path) {
  ...
  // Skip empty TSVs (no data rows after header and type row)
  if (!tsv[0]) return
